...
const unsupportedGenericPhrases = [
  "review github push",
  "review pr #",
  ...
  "github changes should remain review-first"
];

const patchContainsUnsupportedGenericContent = (patch: GitHubSuggestionPatch): boolean => {
  return contextArrayFields.some((field) =>
    patch[field].some((item) => {
      const normalized = item.toLowerCase();
      return unsupportedGenericPhrases.some((phrase) => normalized.includes(phrase));
    })
  );
};
...
const buildPrompt = (input: GitHubSuggestionRefinementInput): string => {
  return [
    ...
    "Use current ProjectContext only to avoid duplicates and understand naming; do not copy old entries unless this GitHub event changes them.",
    "Only add facts supported by this GitHub event. If uncertain, leave the section empty.",
    "Do not invent backend behavior, durable rules, decisions, constraints, dependencies, or next steps.",
    "Do not add generic product rules, review reminders, or next steps unless explicitly present in the commit or PR.",
    "The word version in a frontend path/title may mean a version-history UI, not backend ContextVersion metadata.",
    "Changed file paths and safe diff summary are primary evidence; commit/PR text is secondary evidence.",
    ...
    "ProjectContext patch sections must contain durable current project memory only, not analysis/debug text.",
    "Dependencies must remain empty unless package/dependency files or diff evidence identify actual dependency names.",
    "Issues must only contain unresolved/current issues; fixed bugs should become completed capabilities or be omitted.",
...
export const githubSuggestionRefinementService = {
  async refine(input: GitHubSuggestionRefinementInput): Promise<GitHubSuggestionRefinementAttempt> {
    ...
    try {
      ...
      const normalizedPatch = normalizePatch(parsed.suggestedPatch);
      if (patchContainsUnsupportedGenericContent(normalizedPatch)) {
        return fallback(input, true, "unsupported_generic_ai_content");
      }
      return {
        attempted: true,
        succeeded: true,
        ...
        result: {
          title: parsed.title,
          suggestedPatch: normalizedPatch,
          confidence: parsed.confidence,
          reasoningSummary: `${parsed.reasoningSummary} AI-refined from evidence-bound GitHub event metadata and current ProjectContext.`,
          refinementUsed: true
        }
      };
    } catch (error) {
